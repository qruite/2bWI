console.log("Hallo");
console.log("Tolle Sache");

let a = 7;
let b = 8;
let c = 10;

// Berechne folgendes und gib das Eregebnis über console.log aus
let result = a + b + c;
console.log("7 + 8 + 10 = " + result)

result = (a + b) / c;
console.log("(7 + 8) / 10 = " + result)
 
result = a + b + c * 3;
console.log("7 + 8 + 10 * 3 = " + result);

result = (a + b + c)/4;
console.log("(7 + 8 + 10) / 4 = " + result);

// Ändere die Variablen auf folgende Werte
a = 3
b = a + 3
c = c -3

// gib die neuen Werte der Variablen aus
console.log("a ist gleich: " + a);
console.log("b ist gleich: " + b);
console.log("c ist gleich: " + c);